...
class IREM_API_Request {
	...
	static function verify_request() {
		...
		if ( ! iremo_get_site_keys() ) {
			...
			if ( (int) $_POST['timestamp'] > time() + 86400 || (int) $_POST['timestamp'] < time() - 86400 ) {
...
foreach( IREM_API_Request::get_actions() as $action ) {
	...
	switch( $action ) {
		...
		$actions[$action]     = array(
			'revisions'          	=> iRemoteWPSweep::get_instance()->countsweep( 'revisions' ),
			'auto_drafts'        	=> iRemoteWPSweep::get_instance()->countsweep( 'auto_drafts' ),
			'deleted_posts'        	=> iRemoteWPSweep::get_instance()->countsweep( 'deleted_posts' ),
			'unapproved_comments'  	=> iRemoteWPSweep::get_instance()->countsweep( 'unapproved_comments' ),
			'duplicated_postmeta'  	=> iRemoteWPSweep::get_instance()->countsweep( 'duplicated_postmeta' ),
			'unused_terms'  		=> iRemoteWPSweep::get_instance()->countsweep( 'unused_terms' ),
			'transient_options'  	=> iRemoteWPSweep::get_instance()->countsweep( 'transient_options' )
		);
		...
		$actions[$action]     = array(
			'revisions'          	=> iRemoteWPSweep::get_instance()->sweep( 'revisions' ),
			'auto_drafts'        	=> iRemoteWPSweep::get_instance()->sweep( 'auto_drafts' ),
			'deleted_posts'        	=> iRemoteWPSweep::get_instance()->sweep( 'deleted_posts' ),
			'unapproved_comments'  	=> iRemoteWPSweep::get_instance()->sweep( 'unapproved_comments' ),
			'duplicated_postmeta'  	=> iRemoteWPSweep::get_instance()->sweep( 'duplicated_postmeta' ),
			'unused_terms'  		=> iRemoteWPSweep::get_instance()->sweep( 'unused_terms' ),
			'transient_options'  	=> iRemoteWPSweep::get_instance()->sweep( 'transient_options' )
		);
	...
	$_credentials = array(
        ...
        case 'validate_plugin' :
            $actions[$action] = _iremo_validate( sanitize_text_field( IREM_API_Request::get_arg( 'plugin' ) ) );
        break;
		...
		case 'send2dropboxAUTH' :

			$actions[$action] = IREMOTE_Backups::get_instance()->send2dropboxAUTH(IREM_API_Request::get_arg( 'dropboxauth' ));
		...
		case 'remove_security':
			...
			delete_site_option( '_iremo_ipfilter_ftype' );
			delete_site_option( '_iremo_ipfilter_ips' );
			delete_site_option( '_iremo_ipfilter_bypass_url' );
			$actions[$action] = true;
		...
		case 'get_option':
			...
			$actions[$action] = get_site_option( sanitize_text_field( IREM_API_Request::get_arg( 'option_name' ) ) );

			break;

		case 'update_option':

			$actions[$action] = update_site_option( sanitize_text_field( IREM_API_Request::get_arg( 'option_name' ) ), IREM_API_Request::get_arg( 'option_value' ) );

		break;

		case 'delete_option':

			$actions[$action] = delete_site_option( sanitize_text_field( IREM_API_Request::get_arg( 'option_name' ) ) );
