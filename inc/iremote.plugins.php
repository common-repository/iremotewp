 ...
 */
function _iremo_update_plugin_wrap( $plugin_file, $args )
{
    ...
    try {
        $response = _iremo_update_plugin($plugin_file, $args);
    ...
    if ( ! file_exists(WP_PLUGIN_DIR . '/' . $plugin_file) ){
        ...
        _iremo_install_plugin($plugin_slug);
...
function _iremo_update_plugin( $plugin_file, $args ) {
	...
	require_once IREMOTE_PLUGIN_PATH . 'inc/class-iremote-automatic-upgrader-skin.php';
    ...
    ob_start();
    $skin2 = new IREMOTE_Automatic_Upgrader_Skin();
    ...
    if ( ! empty( $skin->error ) ) {
        ...
        if ($skin->error == 'up_to_date') {
            return new WP_Error('up_to_date', __('Plugin already up to date.', 'iremotewp'));
        }
        $msg = __('Unknown error updating plugin.', 'iremotewp');
 ...
 * @return array|WP_Error
 */
function _iremo_validate($plugin_file)
{
    $plugin_status = false;
    foreach( get_plugins() as $path => $maybe_plugin ) {
        if ( $path == $plugin_file ) {
    ...
    if (!$plugin_status) {
        return new WP_Error('plugin-missing', __('Plugin has gone missing.', 'iremotewp'));
    }
    return array(
        'status' => 'success',
