...
function _iremo_set_security($ftype, $ips_list, $bypass_url) {
		...
		{
				update_site_option( '_iremo_ipfilter_ftype', $ftype );
		}
		else
		{
		...
		{
			update_site_option( '_iremo_ipfilter_ips', serialize($ips_list) );
		}
		else
		{
		...
		{
			update_site_option( '_iremo_ipfilter_bypass_url', $bypass_url );
		}
		else
		{
	...
	global $pagenow;
	if( 'wp-login.php' == $pagenow  || 'admin-ajax.php' == $pagenow  || 'xmlrpc.php' == $pagenow ) {

	       if(get_site_option( '_iremo_ipfilter_ips' )) {

				$ip_filters = unserialize( get_site_option( '_iremo_ipfilter_ips' ) );
					// if grant
					if(get_site_option( '_iremo_ipfilter_ftype' ) == 'grant' ){

						if(!in_array(get_visitor_ips(), $ip_filters)){
							_iremo_redir_url();
						}

					} else if(get_site_option( '_iremo_ipfilter_ftype' ) == 'deny' ){
	...
	function _iremo_redir_url(){
					...
					if($redir_url = get_site_option( '_iremo_ipfilter_bypass_url' )){
						//wp_die( 'die' );
